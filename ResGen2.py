...
def ResGen2(z,state,params,Ts):
    """ RESGEN2 Sequential residual generator for model ''
    Causality: int
    ...
    for k,zk in enumerate(z):
        r[k], state = ResGen2( zk, state, params, Ts )

    State is a dictionary with the keys: x1, x2

    File generated Fri Apr 26 10:46:26 2024
    """
    def ApproxInt(dx, x0, Ts):
        return x0 + Ts*dx

    def ResGen2_core(z, state, params, Ts):
        # Known signals
        y1 = z[0]
        y3 = z[1]
        u = z[2]
    ...
    return ResGen2_core(z, state, params, Ts)
